...
public class MedecinDao {
	...
	public static Medecin rechercher(String codeMedecin){
		...
		ResultSet reqSelection = ConnexionMySql.execReqSelection("select * from `javaoracle_MEDECIN` where CODEMED ='"+codeMedecin+"'");
		try {
	...
	public static ArrayList<Medecin> retournerCollectionDesMedecins(){
		...
		ResultSet reqSelection = ConnexionMySql.execReqSelection("select CODEMED from `javaoracle_MEDECIN`");
		try{
	...
	public static void Retraiter(String Codemed) {
		ConnexionMySql.execReqSelection("UPDATE `javaoracle_MEDECIN` SET `RETRAITE` = '0' WHERE `CODEMED` = '"+ Codemed +"';");
		ConnexionMySql.fermerConnexionBd();
	...
	public static void Archiver() {
		...
		ConnexionMySql.execReqSelection("call Purge_bas");
		ConnexionMySql.fermerConnexionBd();
	}
