...
public class VisiteDao {
	...
	public static Visite rechercher(String references) {
		...
		//Requete SQL
		ResultSet reqSelection = ConnexionMySql.execReqSelection("select * from `javaoracle_VISITE` where REFERENCE ='"+references+"'");
		System.out.println("select * from `javaoracle_VISITE` where REFERENCE ='"+references+"'");
	...
	public static ArrayList<Visite> retournerCollectionLesVisites() {
		...
		ResultSet reqSelection = ConnexionMySql.execReqSelection("select REFERENCE from `javaoracle_VISITE`;");
		//test de la requete 
		try {
